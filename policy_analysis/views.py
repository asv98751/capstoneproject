...
# GPT-3.5 Key Changes Function
def key_changes_gpt(current_text, previous_text):
    messages = [
        {"role": "system", "content": """You are an assistant that summarizes key changes between privacy policies.
        Compare the provided previous and current policies and summarize the most significant changes. Use bullet points 
        and keep it brief (2-3 points)."""},
        {"role": "user", "content": f"""The previous privacy policy version is:
    {previous_text}

    The current privacy policy version is:
    {current_text}

    Summarize the key changes between these two versions."""}
        ]
    ...
    try:
        ...
        return f"Error generating key changes: {e}"
...
def site_detail(request, site_id):
    ...
    # Step 1: Deserialize privacy_policy_history safely
    privacy_policy_history = site.privacy_policy_history or []
    if isinstance(privacy_policy_history, str):
        try:
            privacy_policy_history = json.loads(privacy_policy_history)
        except json.JSONDecodeError:
            privacy_policy_history = []  # Default to empty list on error

    # Step 2: Ensure history is sorted and deduplicated
    seen_dates = set()
    filtered_history = []

    for entry in sorted(privacy_policy_history, key=lambda x: x.get('date', ''), reverse=True):
        if isinstance(entry, dict) and entry.get("date") and entry["date"] not in seen_dates:
            seen_dates.add(entry["date"])
            filtered_history.append(entry)

    # Step 3: Compare current policy with the latest previous version
    key_changes = "No previous version available; hence no key changes."
    if len(filtered_history) >= 1:
        previous_policy = filtered_history[0].get("privacy_policy_extracted", "")
        current_policy = site.privacy_policy_extracted or ""

        if previous_policy and current_policy:
            key_changes = key_changes_gpt(current_policy, previous_policy)

    # Step 4: Auto Scan Countdown Logic
    remaining_seconds = 0
    if site.auto_scan_enabled:
        now = timezone.now()
        next_midnight = now.replace(hour=0, minute=0, second=0, microsecond=0) + timezone.timedelta(days=1)
    ...
    context = {
        ...
        'key_changes': markdown.markdown(key_changes),  # Convert to HTML
...
def add_policy_to_history(self, new_policy, new_summary, new_date):
    """
    Add key changes and summaries to the privacy policy history.
    """
    ...
    # Fetch previous policy text for comparison
    ...
    # Check for changes in policy content or date
    if new_date.strftime('%Y-%m-%d') != last_date or new_policy.strip() != last_policy.strip():
        # Generate key changes using the previous and current policies
        key_changes = key_changes_gpt(new_policy.strip(), last_policy.strip())

        # Create the history entry
        history_entry = {
            "date": new_date.strftime('%Y-%m-%d'),
            "key_changes": key_changes.strip(),
